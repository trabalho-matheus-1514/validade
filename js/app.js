(function () {
  ...
  function rowToProduct(r) {
    return {
      id: r.id, barcode: r.barcode || "", name: r.name, category: r.category || "Outros",
  ...
  function movementToRow(m) {
    return {
      product_id: m.productId, product_name: m.productName, type: m.type,
  ...
  function loadFromSupabase() {
    return Promise.all([
      supa.from("products").select("*").order("expiry_date", { ascending: true }),
      supa.from("movements").select("*").order("created_at", { ascending: false })
  ...
  /* ---- Realtime: reflete mudanças feitas em outros dispositivos ---- */
  function subscribeRealtime() {
    supa.channel("public:products")
    ...
    supa.channel("public:movements")
      .on("postgres_changes", { event: "*", schema: "public", table: "movements" }, function (payload) {
  ...
  function applyProductRealtimeEvent(payload) {
    if (payload.eventType === "INSERT" || payload.eventType === "UPDATE") {
      upsertProductLocal(rowToProduct(payload.new));
  ...
  function applyMovementRealtimeEvent(payload) {
    if (payload.eventType === "INSERT") {
      upsertMovementLocal(rowToMovement(payload.new));
    } else if (payload.eventType === "DELETE") {
      var removedId = payload.old && payload.old.id;
    ...
    supa.from("products").insert(demo.products.map(productToRow)).select()
      .then(function (res) {
        ...
        var movRows = demo.movements.map(function (m) {
          ...
          return real ? {
            product_id: real.id, product_name: m.productName, type: m.type,
            quantity: m.quantity, previous_stock: m.previousStock, new_stock: m.newStock, note: m.note
          } : null;
        }).filter(Boolean);
  ...
  function wireMoveModal() {
    ...
    document.getElementById("move-form").addEventListener("submit", function (e) {
      ...
      var movement = {
        productId: p.id, productName: p.name, type: currentType,
