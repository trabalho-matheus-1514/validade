(function () {
  ...
  function emptyProduct() {
    return {
      ...
      stock: 0, minStock: 5, manufactureDate: "", expiryDate: "", supplier: "", notes: "", createdAt: null
    };
  }
  ...
  function renderProdutosList() {
    ...
    var rows = filtered.map(function (p) {
      ...
      return (
        "<tr>" +
        "<td><div style=\"font-weight:700\">" + escapeHtml(p.name) + '</div><div class="pc-mono pc-muted" style="font-size:11.5px">' + (p.barcode ? escapeHtml(p.barcode) : "sem código") + "</div></td>" +
  ...
  function renderProductModal() {
    ...
    return (
      ...
      field("Código de barras",
        ...
        '<input class="pc-input pc-mono" id="pf-barcode" placeholder="Ex.: 7891234567890" value="' + escapeHtml(p.barcode) + '"/>' +
  ...
  function wireProductModal() {
    var close = function () { state.productModal = null; render(); };
    ...
    document.getElementById("pf-lookup").addEventListener("click", function () {
      var barcode = document.getElementById("pf-barcode").value;
      if (!barcode) return;
      ...
      fetch("https://world.openfoodfacts.org/api/v2/product/" + barcode + ".json")
        ...
        .then(function (json) {
          if (json && json.status === 1 && json.product) {
            ...
            var name = prod.product_name_pt || prod.product_name;
            var brand = (prod.brands || "").split(",")[0].trim();
            var cat = guessCategory(prod.categories);
            if (name) document.getElementById("pf-name").value = name;
            if (brand) document.getElementById("pf-brand").value = brand;
            if (cat) document.getElementById("pf-category").value = cat;
            msg.textContent = "Produto encontrado — confira os dados antes de salvar.";
          } else {
            msg.textContent = "Código não encontrado. Preencha os dados manualmente.";
          }
        })
        .catch(function () {
          msg.textContent = "Não foi possível consultar agora. Preencha os dados manualmente.";
        })
